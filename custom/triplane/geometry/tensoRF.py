...
from .utils import contract_to_unisphere_triplane, get_tensoRF
from .tucker import TriplaneTucker


@threestudio.register("triplane-tensoRF")
class TriplaneTensoRF(BaseImplicitGeometry):
    ...
    def configure(self) -> None:
        super().configure()
        self.encoding = get_tensoRF(
            self.cfg.n_input_dims, self.cfg.pos_encoding_config
        )
        # self.density_network = get_mlp(
        #     self.encoding.n_output_dims, 1, self.cfg.mlp_network_config
        # )
        self.density_network = nn.Identity()
    ...
    def create_from(
        ...
        **kwargs,
        ...
        if isinstance(other, TriplaneTensoRF):
            instance = TriplaneTensoRF(cfg, **kwargs)
            ...
            raise TypeError(
                f"Cannot create {TriplaneTensoRF.__name__} from {other.__class__.__name__}"
            )
    ...
    def forward(
        ...
        density = self.encoding.compute_densityfeature(points.view(-1, self.cfg.n_input_dims)).view(*points.shape[:-1], 1)
