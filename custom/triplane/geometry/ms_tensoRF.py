...
from .utils import contract_to_unisphere_triplane, get_ms_tensoRF
from .tucker import TriplaneTucker


@threestudio.register("triplane-ms-tensoRF")
class TriplaneMultiscaleTensoRF(BaseImplicitGeometry):
    ...
    def configure(self) -> None:
        super().configure()
        self.encoding = get_ms_tensoRF(
            self.cfg.n_input_dims, self.cfg.pos_encoding_config
        )
        self.density_network = get_mlp(
            self.encoding.n_density_output_dims, 1, self.cfg.mlp_network_config
        )
    ...
    def create_from(
        ...
        **kwargs,
        ...
        if isinstance(other, TriplaneMultiscaleTensoRF):
            ...
            raise TypeError(
                f"Cannot create {TriplaneMultiscaleTensoRF.__name__} from {other.__class__.__name__}"
            )
    ...
    def forward(
        ...
        enc = self.encoding.compute_appfeature(points.view(-1, self.cfg.n_input_dims))
        density_enc = self.encoding.compute_densityfeature(points.view(-1, self.cfg.n_input_dims))
        density = self.density_network(density_enc).view(*points.shape[:-1], 1)
    ...
    def forward_density(self, points: Float[Tensor, "*N Di"]) -> Float[Tensor, "*N 1"]:
        ...
        points = contract_to_unisphere_triplane(points_unscaled, self.bbox, self.unbounded)
        density = self.density_network(
            self.encoding.compute_densityfeature(points.reshape(-1, self.cfg.n_input_dims))
