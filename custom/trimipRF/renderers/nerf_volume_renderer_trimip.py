...
def compute_ball_radii(distance, radiis, cos):
    # breakpoint()
    inverse_cos = 1.0 / cos 
    tmp = (inverse_cos ** 2 - 1).sqrt() - radiis 
    sample_ball_radii = distance * radiis * cos / (tmp ** 2 + 1.0).sqrt()
    return sample_ball_radii 

def compute_radis_and_cos(height, width, fovy, batch_size):
    """
    ...
    dx = torch.cat([dx, dx[:, :, -2:-1, :]], -2)  # [B, H,W,1]
    dy = torch.linalg.norm(
        (directions[:, :-1, :, :] - directions[:, 1:, :, :]),
        dim=-1,
        keepdims=True,
    )  # [H-1,W,1]
    dy = torch.cat([dy, dy[:, -2:-1, :, :]], -3)  # [B, H,W,1]
    area = dx * dy
    ...
    ray_cos=torch.matmul(
        directions,
        torch.tensor([[0.0, 0.0, -1.0]], device=directions.device).T,
    )
    return radii, ray_cos


@threestudio.register("nerf-volume-renderer-trimip")
class NeRFVolumeRendererTriMip(VolumeRenderer):
    @dataclass
    ...
    def configure(
        ...
        if self.cfg.estimator == "occgrid":
            ...
            self.register_buffer("occ_level_vol", torch.log2(torch.tensor(1/32)))
    ...
    def forward(
        ...
        **kwargs
        ...
        radiis, ray_cos = compute_radis_and_cos(height, width, kwargs["fovy"], batch_size) 
        radiis = radiis.reshape(-1, 1) 
        ray_cos = ray_cos.reshape(-1, 1)
        ...
        if self.cfg.estimator == "occgrid":
            if not self.cfg.grid_prune:
                ...
                def sigma_fn(t_starts, t_ends, ray_indices, radiis, ray_cos):
                    t_starts, t_ends = t_starts[..., None], t_ends[..., None]
                    t_origins = rays_o_flatten[ray_indices]
                    t_positions = (t_starts + t_ends) / 2.0
                    t_dirs = rays_d_flatten[ray_indices]
                    positions = t_origins + t_dirs * t_positions
                    radiis = radiis[ray_indices]
                    cos = ray_cos[ray_indices]
                    sample_ball_radii = compute_ball_radii(
                ...
                with torch.no_grad():
                    ray_indices, t_starts_, t_ends_ = self.estimator.sampling(
                        rays_o_flatten,
                        rays_d_flatten,
                        sigma_fn=lambda t_s, t_e, r_i: sigma_fn(t_s, t_e, r_i, radiis=radiis, ray_cos=ray_cos) if self.cfg.prune_alpha_threshold else None,
                        near_plane=self.cfg.near_plane,
                        far_plane=self.cfg.far_plane,
                        render_step_size=self.render_step_size,
        ...
        if self.training:
            geo_out = self.geometry(
                positions, level_vol, output_normal=self.material.requires_normal
            )
