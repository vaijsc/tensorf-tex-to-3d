# Want to implement TriMipRF model
# - encoding 
    # for each point in ray, we need another radius to select level of feature map
# - forward_density method
...
@threestudio.register("trimip-rf")
class TriMipRFModel(BaseImplicitGeometry):
    @dataclass
    ...
    def forward(
        ...
        level_vol_added = level_vol + self.encoding.log2_plane_size

        enc = self.encoding(points.view(-1, self.cfg.n_input_dims), level_vol_added.reshape(-1, 1))
            ...
            if (
                ...
                if self.cfg.normal_type == "finite_difference_laplacian":
                    ...
                    density_offset: Float[Tensor, "... 6 1"] = self.forward_density(
                        points_offset,
                        torch.stack([level_vol, level_vol, level_vol], dim=-1)
                    )
                    normal = (
                        -0.5
    ...
    def forward_density(self, 
                        ...
                        level_vol: Float[Tensor, "*N 1"]) -> Float[Tensor, "*N 1"]:
        points_unscaled = points
        points = contract_to_unisphere(points_unscaled, self.bbox, self.unbounded)
        
        level_vol += self.encoding.log2_plane_size
        
        density = self.density_network(
            self.encoding(points.reshape(-1, self.cfg.n_input_dims),
                          level_vol.reshape(-1, 1))
    ...
    def create_from(
        ...
        **kwargs,
        ...
        if isinstance(other, TriMipRFModel):
            instance = TriMipRFModel(cfg, **kwargs)
            ...
            raise TypeError(
                f"Cannot create {TriMipRFModel.__name__} from {other.__class__.__name__}"
            )
